<table>
	...
	{foreach from=$data item=eachRecord}
		...
		<td><a title="Edit {$eachRecord.display_name}'s contact record." href="/civicrm/contact/add?reset=1&action=update&cid={$eachRecord.contact_id}">{$eachRecord.contact_id}</a></td>
		...
		<td>
			<select phone_id="{$eachRecord.phone_id}" selectedValue="{$eachRecord.phone_type}">
				{crmAPI var="OptionValueS" entity="OptionValue" action="get" sequential="1" option_group_id="34" }
